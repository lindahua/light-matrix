...
namespace lmat {
	...
	fold_impl(const dimension<N>& dim, simd_<SKind>, const Folder& folder, const Reader& rd)
	{
		...
		typedef typename simd_folder_t::value_type pack_t;

		simd_folder_t pk_folder = simdize_map<Folder, SKind>::get(folder);
		...
		if (npacks)
		{
			...
			if (m4)
			{
				...
				a0 = pk_folder.init(rd.pack(0));
				a1 = pk_folder.init(rd.pack(pw));
				a2 = pk_folder.init(rd.pack(pw * 2));
				a3 = pk_folder.init(rd.pack(pw * 3));

				for (i = w4; i < l4; i += w4)
				{
					pk_folder.fold(a0, rd.pack(i));
					pk_folder.fold(a1, rd.pack(i + pw));
					pk_folder.fold(a2, rd.pack(i + pw * 2));
					pk_folder.fold(a3, rd.pack(i + pw * 3));
				}

				pk_folder.fold(a0, a2);
				pk_folder.fold(a1, a3);
